...
$app->get('/app/retrievetopcomment',function($request){
	include __DIR__ . '/../bootstrap/dbconnect.php';
		
		$postId = $request->getQueryParam('postId');
		$response=array();
		$comment = retrieveTopLevelComment($postId);
...
$app->get('/app/retrievelowlevelcomment',function($request){
	include __DIR__ . '/../bootstrap/dbconnect.php';
		...
		$response=array();
		$comment = retrieveLowLevelComment($postId,$commentId);
		$response=$comment;
		echo json_encode($response);
				
});
function retrieveTopLevelComment($postId){
		include __DIR__ . '/../bootstrap/dbconnect.php';
		$stmt = $pdo->prepare("
		...
		$comments= $stmt->fetchAll(PDO::FETCH_ASSOC);
			foreach ($comments as $key => $comment) {
				...
				if($comment['hasSubComment']==1){
					...
					$subComments['lastComment']=retrieveLastComment($postId,$comment['cid']);
					$subComments['total']= retrieveTotalCommentCount($postId,$comment['cid']);
	...
	function retrieveLowLevelComment($superParentId,$commentId){
		include __DIR__ . '/../bootstrap/dbconnect.php';
		$stmt = $pdo->prepare("
				SELECT comments.*,users.name,users.profileUrl,users.userToken
				FROM `comments`
...
function retrieveLastComment($postId,$parentComment){
		include __DIR__ . '/../bootstrap/dbconnect.php';
	...
	function retrieveTotalCommentCount($postId,$parentComment){
		include __DIR__ . '/../bootstrap/dbconnect.php';
